...
def build_resnext():
    import pretrainedmodels
    model = pretrainedmodels.__dict__['resnext101_64x4d'](num_classes=1000, pretrained='imagenet')
    model.cuda()
    model.eval()
    net = model.features
...
def build_resnet():
    ...
    from ops.resnet import resnet152
...
def build_i3d():
    sys.path.append('/home/mhy/Documents/code/pytorch-resnet3d')
    from models.resnet import i3_res50_nl
    from util.util import clip_transform
    net_ori  = i3_res50_nl(400)
    state_dict = torch.load('/home/mhy/Documents/code/pytorch-resnet3d/pretrained/i3d_r50_nl_kinetics.pth')
...
def build_tsn():
    ...
    # this_weights = '../temporal-shift-module/pretrained/TSM_kinetics_RGB_resnet50_shift8_blockres_avg_segment16_e50.pth'
    this_weights = '../temporal-shift-module/pretrained/TSM_kinetics_RGB_resnet50_shift8_blockres_avg_segment8_e100_dense_nl.pth'
    # this_weights = '../temporal-shift-module/pretrained/TSM_kinetics_RGB_resnet50_shift8_blockres_avg_segment16_e50_nl.pth'
...
def get_net():
    ...
    if model=='vgg': 
        net, transform = build_vgg()
    if model=='resnet':
        net, transform = build_resnet()
    if model=='incep_res':
        net, transform = build_incep_res()
    if model=='inception':
        net, transform = build_inception()
    if model=='resnext':
        net, transform = build_resnext()
    if model=='senet':
        net, transform = build_senet()
    if model=='pnasnet':
        net, transform = build_pnasnet()
    elif model=='i3d':
...
if __name__ == '__main__':
    ...
    avg_pool = torch.nn.AdaptiveAvgPool2d(output_size).cuda()
    if mode=='clip':
        avg_pool = torch.nn.AdaptiveAvgPool3d(output_size).cuda()
    max_pool = torch.nn.AdaptiveMaxPool2d((1, 1)).cuda()
    ...
    for idx in tqdm.tqdm(range(start_idx, len(videos_list))):
        ...
        with torch.no_grad():
            ...
            for i in range(sample_len):  
                ...
                bbox_det_single[:, 0] = i
                bbox.append(bbox_det_single)
            bbox = torch.cat(bbox, dim=0).cuda()

            if mode=='frame':
                box_feat = roi(base_feat, bbox)  # n, C, 16, 16
            elif mode=='clip':
                box_feat = roi(base_feat[:,:,0], bbox)  # n, C, 16, 16
            
            box_feat = max_pool(box_feat).squeeze(-1).squeeze(-1)  # n, C    
            frame_feat = avg_pool(base_feat).squeeze(-1).squeeze(-1).squeeze(-1)
