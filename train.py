...
# from models.vt_model import S2VT
# from models.vt_attention import S2VT_Attention
# from models.vt_transformer import S2VT_Transformer
...
def main(opt):
    ...
    print("dataset: %s"%opt["dataset"], 
          ...
          "saveing checkpoint to: %s"%opt["checkpoint_path"], 
          "start training", 
          "---------------------------", sep="\n")
    ...
    for epoch in range(opt["epochs"]):
        ...
        with torch.no_grad():
            ...
            if epoch+1 >= opt["eval_start"]:
                ...
                with open(sents_json + '/valid_%s.json'%(epoch), 'w') as f:
            ...
            if is_best:
                ...
                print_info.append("test_CIDEr: {:.1f}".format(test_score["CIDEr"]))
                writer.add_scalar('Bleu_4/test', test_score["Bleu_4"], epoch)
                writer.add_scalar('METEOR/test', test_score["METEOR"], epoch)
                writer.add_scalar('CIDEr/test', test_score["CIDEr"], epoch)
                ...
                with open(sents_json + '/test_%s.json'%(epoch), 'w') as f:
        ...
        if (epoch+1) % 10 == 0:
            print('tensorboard --logdir={}'.format(os.path.join(os.getcwd(), opt["checkpoint_path"])))

    # select the top result
    top = metrics_steady.sort_values(by=['CIDEr'], ascending=False)[:top_metics]
    top = list(top['epoch'])
    test_top = metrics_test.loc[metrics_test['epoch'].isin(top)]
    test_top.to_csv(metrics_log_test_top, float_format='%.1f', index=False)
...
if __name__ == '__main__':
    ...
    top_metics = 10
    steady_epoch = opt["self_crit_after"]  # 700
    ...
    sents_json = os.path.join(opt["checkpoint_path"], 'sents')
    ...
    # log files
    loss_log_file = os.path.join(opt["checkpoint_path"], 'loss.csv')
    metrics_log_valid = os.path.join(opt["checkpoint_path"], 'metrics_valid.csv')
    metrics_log_test = os.path.join(opt["checkpoint_path"], 'metrics_test.csv')
    metrics_log_test_top = os.path.join(opt["checkpoint_path"], 'metrics_test_top.csv')
