...
def main(params):
    ...
    dataset = 'MSRVTT' if 'MSRVTT' in params['input_json'] else 'MSVD'
    video_caption = {}
    for i in videos:
    ...
    # unique_filenames = json_normalize(videos)['video_id'].unique()
    # for i in videos:
    #     out['videos'][i['split']].append(int(i['id']))
    info_json = 'data/info_.json'.format(dataset)
    caption_json = 'data/caption_.json'.format(dataset)
    json.dump(out, open(info_json, 'w'))
    json.dump(video_caption, open(caption_json, 'w'))
...
if __name__ == "__main__":
    ...
    # input json
    parser.add_argument('--input_json', type=str, default='data/annotation_MSVD.json',
