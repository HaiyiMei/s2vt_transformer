...
class Encoder_lstm(nn.Module):
    ...
    def forward(self, input_image, input_box=None):
        ...
        if self.fusion:
            ...
            mask = self.region_mask(input_image) if self.only_box else None  # batch_size x N
            ...
            if mask is not None:
                input_feature = input_feature.sum(0) / (~mask).sum(-1).unsqueeze(-1).clamp(min=1)
            else:
                input_feature = input_feature.mean(0)
